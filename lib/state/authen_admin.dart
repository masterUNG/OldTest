...
import 'package:joelfindtechnician/utility/my_condtant.dart';
...
class _AuthenAdminState extends State<AuthenAdmin> {
  ...
  Future<Null> checkAuthen() async {
    ...
    }).catchError((value) => MyDialog().normalDialog(context, value.code, value.message));
  }

  ElevatedButton buildLogin() => ElevatedButton(
      onPressed: () {
        if (formKey.currentState!.validate()) {
          ...
          if (status) {
            ...
            MyDialog()
                .normalDialog(context, 'Email False', 'Email ไม่ใช่ Admin');
          }
        }
      },
      child: Text('Login'));
